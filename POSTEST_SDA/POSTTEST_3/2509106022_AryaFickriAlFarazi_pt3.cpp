...
bool isQueueFull(const Queue *q) { return q->ukuran == MAX_QUEUE; }
bool isQueueEmpty(const Queue *q) { return q->ukuran == 0; }
...
void tampilSemuaAntrian(const Queue *q)
{
    ...
    cout << "Isi antrian (" << q->ukuran << "/" << MAX_QUEUE << ") - front ke rear:\n";
    garis();
    ...
    {
        cetakBarisPemesanan(q->data + idx, i + 1);
        idx = (idx + 1) % MAX_QUEUE;
    }
    garis();
}
...
void beliTiket(Queue *q)
{
    ...
    if (isQueueFull(q))
    {
        cout << "OVERFLOW! Antrian penuh (" << MAX_QUEUE << ").\n";
        return;
    }
    tampilSemuaKereta(dataKereta, jumlahKereta);
    if (jumlahKereta == 0)
...
bool isStackFull(const Stack *s) { return s->top == MAX_STACK - 1; }
bool isStackEmpty(const Stack *s) { return s->top == -1; }
...
void tampilSemuaRiwayat(const Stack *s)
{
    ...
    cout << "Total: " << s->top + 1 << " transaksi (terbaru di atas):\n";
    garis();
    cout << left << setw(4) << "No." << setw(8) << "ID" << setw(20) << "Penumpang"
    ...
    {
        const Pemesanan *p = s->data + i;
        cout << left
             << setw(4) << urutan++
             << setw(8) << p->idPemesanan
             << setw(20) << p->namaPenumpang
             << setw(18) << p->namaKereta
             << setw(22) << (p->asal + "->" + p->tujuan)
             << p->status << "\n";
    }
    garis();
}
...
void prosesTiket(Queue *q, Stack *s)
{
    ...
    if (isStackFull(s))
    {
        cout << "Peringatan: stack riwayat penuh, transaksi tidak tercatat.\n";
    }
    else
    {
        push(s, diproses);
...
void batalkanTransaksiTerakhir(Stack *s)
{
    ...
    cout << "Sisa riwayat: " << s->top + 1 << " transaksi\n";
}
